...
class AttModel(CaptionModel):
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state, output_logsoftmax=1):
        ...
        output1, output2, output3, state = self.core(xt, fc_feats, att_feats, p_att_feats, state, att_masks)
        if output_logsoftmax:
            logprobs1 = F.log_softmax(self.logit1(output1), dim=1)
            logprobs2 = F.log_softmax(self.logit2(output2), dim=1)
            logprobs3 = F.log_softmax(self.logit3(output1), dim=1)
        else:
            logprobs1 = self.logit1(output1)
            logprobs2 = self.logit2(output2)
            logprobs3 = self.logit3(output3)
        logprobs = (logprobs1+logprobs2+logprobs3)/3
