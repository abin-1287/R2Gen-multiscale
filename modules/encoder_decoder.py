...
class Transformer(nn.Module):
    ...
    def decode(self, hidden_states, src_mask, tgt, tgt_mask):
        # hidden_states_49 = hidden_states[:, -98:, :]
        # src_mask_49 = src_mask[:, :, -98:]
        # memory = self.rm.init_memory(hidden_states_49.size(0)).to(hidden_states_49)
        # memory = self.rm(self.tgt_embed(tgt), memory)
        # return self.decoder(self.tgt_embed(tgt), hidden_states_49, src_mask_49, tgt_mask, memory)
        # hidden_states_196 = hidden_states[:, -392:, :]
        # src_mask_196 = src_mask[:, :, -392:]
        # memory = self.rm.init_memory(hidden_states_196.size(0)).to(hidden_states_196)
        # memory = self.rm(self.tgt_embed(tgt), memory)
        # return self.decoder(self.tgt_embed(tgt), hidden_states_196, src_mask_196, tgt_mask, memory)
        hidden_states_784 = hidden_states[:, -1568:, :]
        src_mask_784 = src_mask[:, :, -1568:]
        memory = self.rm.init_memory(hidden_states_784.size(0)).to(hidden_states_784)
        memory = self.rm(self.tgt_embed(tgt), memory)
        return self.decoder(self.tgt_embed(tgt), hidden_states_784, src_mask_784, tgt_mask, memory)
