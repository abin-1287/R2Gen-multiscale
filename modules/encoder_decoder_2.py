...
class Transformer(nn.Module):
    def __init__(self, encoder, decoder1, decoder2, decoder3, src_embed, tgt_embed, rm):
    ...
    def decode(self, hidden_states, src_mask, tgt, tgt_mask):
        hidden_states1, hidden_states2, hidden_states3 = torch.split(hidden_states, [1568, 392, 98], dim=1)
        src_mask1, src_mask2, src_mask3 = torch.split(src_mask, [1568, 392, 98], dim=2)
        memory1 = self.rm.init_memory(hidden_states1.size(0)).to(hidden_states1)
        memory1 = self.rm(self.tgt_embed(tgt),memory1)
        memory2 = self.rm.init_memory(hidden_states2.size(0)).to(hidden_states2)
        memory2 = self.rm(self.tgt_embed(tgt),memory2)
        memory3 = self.rm.init_memory(hidden_states3.size(0)).to(hidden_states3)
        memory3 = self.rm(self.tgt_embed(tgt),memory3)
        output1 = self.decoder1(self.tgt_embed(tgt), hidden_states1, src_mask1, tgt_mask, memory1)
        output2 = self.decoder2(self.tgt_embed(tgt), hidden_states2, src_mask2, tgt_mask, memory2)
        output3 = self.decoder3(self.tgt_embed(tgt), hidden_states3, src_mask3, tgt_mask, memory3)
...
class EncoderDecoder(AttModel):
    ...
    def __init__(self, args, tokenizer):
        ...
        self.logit2 = nn.Linear(args.d_model, tgt_vocab)
        self.logit3 = nn.Linear(args.d_model, tgt_vocab)
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        out1, out2, out3 = self.model(att_feats, seq, att_masks, seq_mask)
        out1 = F.log_softmax(self.logit1(out1), dim=-1)
        out2 = F.log_softmax(self.logit2(out2), dim=-1)
        out3 = F.log_softmax(self.logit3(out3), dim=-1)
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, state, mask):
        ...
        out1, out2, out3 = self.model.decode(memory, mask, ys, subsequent_mask(ys.size(1)).to(memory.device))

        return out1[:, -1], out2[:, -1], out3[:, -1], [ys.unsqueeze(0)]
